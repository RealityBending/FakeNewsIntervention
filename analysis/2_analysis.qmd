---
title: "FaceNewsIntervention - Data Analysis"
editor: source
editor_options: 
  chunk_output_type: console
format:
  html:
    output-file: "analysis_report.html"
    code-fold: true
    self-contained: true
    toc: true
---


## Data Preparation

```{r}
#| message: false
#| warning: false

library(emmeans)
library(tidyverse)
library(easystats)
library(patchwork)
library(ggside)
```


```{r}
#| code-fold: false

df <- read.csv("../data/data.csv")
dfmist <- read.csv("../data/data_mist.csv")
```

## Analysis


### Example for Continuous Variable

```{r}

m <- lm(MIST_correct_Pretest_covid ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="MIST-Covid (% Correct Pretest)", x="Political Identification")
```

### Example for Categorical Variable


```{r}
# Categorical
m <- lm(MIST_correct_Pretest_covid ~ Political_Affiliation, 
        data=filter(df, Political_Affiliation != "None or Independent"))

display(estimate_contrasts(m, contrast="Political_Affiliation", p_adjust="none"))

estimate_means(m, by = c("Political_Affiliation")) |> 
  ggplot(aes(x=Political_Affiliation, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MIST-Covid (% Correct Pretest)", x="Political Affiliation")
```

--------------------------------------------------------------------------

## Multivariate Model

```{r}
# model <- glmmTMB::glmmTMB(Correct ~ Intervention * Condition * Topic + (1|Participant) + (1|QuestionID), 
#                           data=dfmist, family=binomial())
# 
# model_means <- modelbased::estimate_means(model, by=c("Intervention", "Condition", "Topic"))
# 
# model_means |> 
#   mutate(Condition = fct_rev(Condition)) |> 
#   ggplot(aes(x=Condition, y=Probability, color=Intervention)) +
#   geom_line(aes(group=Intervention), position=position_dodge(width=0.1)) +
#   geom_pointrange(aes(ymin=CI_low, ymax=CI_high), position=position_dodge(width=0.1)) +
#   facet_wrap(~Topic)
```

--------------------------------------------------------------------------

## self-identified political ideology (independent/predictor) & VSA general score (dependent/outcome)

```{r}
m <- lm(VSA_General ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="VSA General Mean Score", x="Political Identification")
```

## self-identified political ideology (independent/predictor) & VSA authoritarianism score (dependent/outcome)

```{r}
m <- lm(VSA_Authoritarianism ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="VSA Authoritarianism Score", x="Political Identification")
```


## self-identified political ideology (independent/predictor) & BFI_Agreeableness (dependent/outcome)

```{r}
m <- lm(BFI_Agreeableness ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="BFI Agreeableness", x="Political Identification")
```

## self-identified political ideology (independent/predictor) & BFI_Extraversion (dependent/outcome)

```{r}
m <- lm(BFI_Extraversion ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="BFI Extraversion", x="Political Identification")
```

## self-identified political ideology (independent/predictor) & BFI_Conscientiousness (dependent/outcome)

```{r}
m <- lm(BFI_Conscientiousness ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="BFI Conscientiousness", x="Political Identification")
```

## self-identified political ideology (independent/predictor) & BFI_Neuroticism (dependent/outcome)

```{r}
m <- lm(BFI_Neuroticism ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="BFI_Neuroticism", x="Political Identification")
```

## self-identified political ideology (independent/predictor) & BFI_Openness (dependent/outcome)

```{r}
m <- lm(BFI_Openness ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="BFI_Openness", x="Political Identification")
```

## self-identified political ideology (independent/predictor) & GCBS_General (dependent/outcome)

```{r}
m <- lm(GCBS_General ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="GCBS_General", x="Political Identification")
```

## self-identified political ideology (independent/predictor) & GCBS_Control (dependent/outcome)

```{r}
m <- lm(GCBS_Control ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="GCBS_Control", x="Political Identification")
```

## self-identified political ideology (independent/predictor) & GCBS_Wellbeing (dependent/outcome)

```{r}
m <- lm(GCBS_Wellbeing ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="GCBS_Wellbeing", x="Political Identification")
```

## self-identified political ideology (independent/predictor) & GCBS_Malevolent (dependent/outcome)

```{r}
m <- lm(GCBS_Malevolent ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="GCBS_Malevolent", x="Political Identification")
```

## self-identified political ideology (independent/predictor) & GCBS_Extraterrestrial (dependent/outcome)

```{r}
m <- lm(GCBS_Extraterrestrial ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="GCBS_Extraterrestrial", x="Political Identification")
```

## self-identified political ideology (independent/predictor) & GCBS_GovernmentMalfeasance (dependent/outcome)

```{r}
m <- lm(GCBS_GovernmentMalfeasance ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="GCBS_GovernmentMalfeasance", x="Political Identification")
```

## MIST COVID (% Correct difference Post-Pre) (independent/predictor) & MIST COVID (% Correct Pretest)

```{r}
m <- lm(MIST_correct_Diff_covid ~ MIST_correct_Pretest_covid, 
        data=filter(df, !is.na(MIST_correct_Pretest_covid)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=MIST_correct_Pretest_covid, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(x="MIST COVID (% Correct pre-test)", y="MIST COVID (% Correct difference Post-Pre)")
```

## MIST General (% Correct difference Post-Pre) (independent/predictor) & MIST General (% Correct Pretest)

```{r}
m <- lm(MIST_correct_Diff_general ~ MIST_correct_Pretest_general, 
        data=filter(df, !is.na(MIST_correct_Pretest_general)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=MIST_correct_Pretest_general, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(x="MIST General (% Correct pre-test)", y="MIST General (% Correct difference Post-Pre)")
```


## self-identified political ideology (independent/predictor) & MIST COVID (% Correct Pre-test) (dependent/outcome)

```{r}
m <- lm(MIST_correct_Pretest_covid ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="MIST COVID (% Correct Pre-test)", x="Political Identification")
```


## self-identified political ideology (independent/predictor) & MIST COVID (dprime Pre-test) (dependent/outcome)

```{r}
m <- lm(MIST_dprime_Pretest_covid ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="MIST COVID (dprime Pre-test)", x="Political Identification")
```


## self-identified political ideology (independent/predictor) & MIST General (% Correct Pre-test) (dependent/outcome)

```{r}
m <- lm(MIST_correct_Pretest_general ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="MIST General (% Correct Pre-test)", x="Political Identification")
```


## self-identified political ideology (independent/predictor) & MIST General (dprime Pre-test) (dependent/outcome)

```{r}
m <- lm(MIST_dprime_Pretest_general ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="MIST General (dprime Pre-test)", x="Political Identification")
```


## self-identified political ideology (independent/predictor) & MIST General (% Correct difference Post-Pre) (dependent/outcome)

```{r}
m <- lm(MIST_correct_Diff_general ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="MIST General (% Correct difference Post-Pre)", x="Political Identification")
```


## self-identified political ideology (independent/predictor) & MIST General (dprime difference Post-Pre) (dependent/outcome)

```{r}
m <- lm(MIST_dprime_Diff_general ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="MIST General (dprime difference Post-Pre)", x="Political Identification")
```


## self-identified political ideology (independent/predictor) & MIST COVID (% Correct difference Post-Pre) (dependent/outcome)

```{r}
m <- lm(MIST_correct_Diff_covid ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="MIST COVID (% Correct difference Post-Pre)", x="Political Identification")
```


## self-identified political ideology (independent/predictor) & MIST COVID (dprime difference Post-Pre) (dependent/outcome)

```{r}
m <- lm(MIST_dprime_Diff_covid ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="MIST COVID (dprime difference Post-Pre)", x="Political Identification")
```


## self-identified political ideology (independent/predictor) & MIST TOTAL (% Correct difference Post-Pre) (dependent/outcome)

```{r}
m <- lm(MIST_correct_Diff ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="MIST TOTAL (% Correct difference Post-Pre)", x="Political Identification")
```

## self-identified political ideology (independent/predictor) & MIST TOTAL (dprime difference Post-Pre) (dependent/outcome)

```{r}
m <- lm(MIST_dprime_Diff ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="MIST TOTAL (dprime difference Post-Pre)", x="Political Identification")
```


## self-identified political ideology (independent/predictor) & MOCRI Pre-test correct (dependent/outcome)

```{r}
m <- lm(MOCRI_correct_Pre ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="MOCRI correct pre-test", x="Political Identification")
```


## self-identified political ideology (independent/predictor) & MOCRI Pre-test dprime (dependent/outcome)

```{r}
m <- lm(MOCRI_dprime_Pre ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="MOCRI dprime pre-test", x="Political Identification")
```


## self-identified political ideology (independent/predictor) & MOCRI difference dprime post-pre (dependent/outcome)

```{r}
m <- lm(MOCRI_dprime_Diff ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="MOCRI dprime difference post-pre", x="Political Identification")
```

## self-identified political ideology (independent/predictor) & MOCRI difference correct post-pre (dependent/outcome)

```{r}
m <- lm(MOCRI_correct_Diff ~ poly(Political_LiberalConservative, 2), 
        data=filter(df, !is.na(Political_LiberalConservative)))

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Political_LiberalConservative, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="MOCRI correct difference post-pre", x="Political Identification")
```

## Gender (independent/predictor) & MIST COVID (% Correct difference post-pre) (dependent/outcome)

```{r}
# Categorical
df$Gender <- as.factor(df$Gender)
m <- lm(MIST_correct_Diff_covid ~ Gender, 
        data=filter(df, Gender != "Non-Binary" & Gender != "Prefer not to answer"))


display(estimate_contrasts(m, contrast="Gender", p_adjust="none"))

estimate_means(m, by = c("Gender")) |> 
  ggplot(aes(x=Gender, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MIST-Covid (% Correct difference post-pre)", x="Gender")
```

## Gender (independent/predictor) & MIST COVID (% Correct pretest) (dependent/outcome)

```{r}
# Categorical
df$Gender <- as.factor(df$Gender)
m <- lm(MIST_correct_Pretest_covid ~ Gender, 
        data=filter(df, Gender != "Non-Binary" & Gender != "Prefer not to answer"))


display(estimate_contrasts(m, contrast="Gender", p_adjust="none"))

estimate_means(m, by = c("Gender")) |> 
  ggplot(aes(x=Gender, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MIST-Covid (% Correct Pre-test)", x="Gender")
```

## Education (independent/predictor) & MIST COVID (% Correct difference post-pre) (dependent/outcome)


```{r}
# Categorical
df$Education <- as.factor(df$Education)
df <- df %>% filter(!is.na(Education))
unique(trimws(df$Education))

m <- lm(MIST_correct_Diff_covid ~ Education, 
        data=filter(df, Education != "Other"))

display(estimate_contrasts(m, contrast="Education", p_adjust="none"))

estimate_means(m, by = c("Education")) |> 
  ggplot(aes(x=Education, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MIST-Covid (% Correct difference post-pre)", x="Education")
```

## Education (independent/predictor) & MIST COVID (% Correct pretest) (dependent/outcome)

```{r}
# Categorical
df$Education <- as.factor(df$Education)
m <- lm(MIST_correct_Pretest_covid ~ Education, 
        data=filter(df, Education != "Other"))

display(estimate_contrasts(m, contrast="Education", p_adjust="none"))

estimate_means(m, by = c("Education")) |> 
  ggplot(aes(x=Education, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MIST-Covid (% Correct Pre-test)", x="Education")
```

## Age (independent/predictor) & MIST COVID (% Correct pretest) (dependent/outcome)

```{r}
m <- lm(MIST_correct_Pretest_covid ~ Age, 
        data=df)

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Age, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="MIST-Covid (% Correct Pretest)", x="Age")
```

## Age (independent/predictor) & MIST COVID (% Correct difference post-pre) (dependent/outcome)

## why does this one show essentially no change in intervention effect size whatsoever? Feels like I made a mistake here somewhere

```{r}
m <- lm(MIST_correct_Diff_covid ~ Age, 
        data=df)

display(parameters(m))

estimate_relation(m, length=30) |> 
  ggplot(aes(x=Age, y=Predicted)) +
  # geom_jitter(data=filter(df, !is.na(Political_LiberalConservative)), 
  #             aes(y=MIST_correct_Pretest_covid), alpha=0.2, size=3) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high), alpha=0.2) +
  geom_line() +
  labs(y="MIST-Covid (% Correct difference post-pre)", x="Age")
```

## Intervention vs Control (independent/predictor) & MOCRI (% Correct difference post-pre) (dependent/outcome)

```{r}
# Categorical
m <- lm(MOCRI_correct_Diff ~ Intervention, 
        data=df)

display(estimate_contrasts(m, contrast="Intervention", p_adjust="none"))

estimate_means(m, by = "Intervention") |> 
  ggplot(aes(x=Intervention, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MOCRI (% Correct difference post-pre)", x="Intervention vs Control")
```

## Intervention vs Control (independent/predictor) & MOCRI (% dprime difference post-pre) (dependent/outcome)

```{r}
# Categorical
m <- lm(MOCRI_dprime_Diff ~ Intervention, 
        data=df)

display(estimate_contrasts(m, contrast="Intervention", p_adjust="none"))

estimate_means(m, by = "Intervention") |> 
  ggplot(aes(x=Intervention, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MOCRI (% dprime difference post-pre)", x="Intervention vs Control")
```


## Intervention vs Control (independent/predictor) & MIST COVID (% Correct difference post-pre) (dependent/outcome)

```{r}
# Categorical
m <- lm(MIST_correct_Diff_covid ~ Intervention, 
        data=df)

display(estimate_contrasts(m, contrast="Intervention", p_adjust="none"))

estimate_means(m, by = "Intervention") |> 
  ggplot(aes(x=Intervention, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MIST-Covid (% Correct difference post-pre)", x="Intervention vs Control")
```

## Intervention vs Control (independent/predictor) & MIST COVID (dprime difference post-pre) (dependent/outcome)

```{r}
# Categorical
m <- lm(MIST_dprime_Diff_covid ~ Intervention, 
        data=df)

display(estimate_contrasts(m, contrast="Intervention", p_adjust="none"))

estimate_means(m, by = "Intervention") |> 
  ggplot(aes(x=Intervention, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MIST-Covid (dprime difference post-pre)", x="Intervention vs Control")
```


## Intervention vs Control (independent/predictor) & MIST general (% Correct difference post-pre) (dependent/outcome)

```{r}
# Categorical
m <- lm(MIST_correct_Diff_general ~ Intervention, 
        data=df)

display(estimate_contrasts(m, contrast="Intervention", p_adjust="none"))

estimate_means(m, by = "Intervention") |> 
  ggplot(aes(x=Intervention, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MIST-general (% Correct difference post-pre)", x="Intervention vs Control")
```

## Intervention vs Control (independent/predictor) & MIST general (dprime difference post-pre) (dependent/outcome)

```{r}
# Categorical
m <- lm(MIST_dprime_Diff_general ~ Intervention, 
        data=df)

display(estimate_contrasts(m, contrast="Intervention", p_adjust="none"))

estimate_means(m, by = "Intervention") |> 
  ggplot(aes(x=Intervention, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MIST-general (dprime difference post-pre)", x="Intervention vs Control")
```


## Intervention vs Control (independent/predictor) & MIST total (% Correct difference post-pre) (dependent/outcome)

```{r}
# Categorical
m <- lm(MIST_correct_Diff ~ Intervention, 
        data=df)

display(estimate_contrasts(m, contrast="Intervention", p_adjust="none"))

estimate_means(m, by = "Intervention") |> 
  ggplot(aes(x=Intervention, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MIST-TOTAL (% Correct difference post-pre)", x="Intervention vs Control")
```


## Intervention vs Control (independent/predictor) & MIST total (dprime difference post-pre) (dependent/outcome)

```{r}
# Categorical
m <- lm(MIST_dprime_Diff ~ Intervention, 
        data=df)

display(estimate_contrasts(m, contrast="Intervention", p_adjust="none"))

estimate_means(m, by = "Intervention") |> 
  ggplot(aes(x=Intervention, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MIST-TOTAL (dprime difference post-pre)", x="Intervention vs Control")
```


## ONLY SELF-IDENTIFIED REPUBLICANS [party affiliation] Intervention vs Control (independent/predictor) & MOCRI (% Correct difference post-pre) (dependent/outcome)

```{r}
df_republicans <- df[df$Political_Affiliation == "Republican", ]

m <- lm(MOCRI_correct_Diff ~ Intervention, 
        data=df_republicans)

display(estimate_contrasts(m, contrast="Intervention", p_adjust="none"))

estimate_means(m, by = "Intervention") |> 
  ggplot(aes(x=Intervention, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MOCRI (% Correct difference post-pre)", x="Intervention vs Control")

```

## ONLY SELF-IDENTIFIED REPUBLICANS [party affiliation] Intervention vs Control (independent/predictor) & MOCRI (dprime difference post-pre) (dependent/outcome)

```{r}
df_republicans <- df[df$Political_Affiliation == "Republican", ]

m <- lm(MOCRI_dprime_Diff ~ Intervention, 
        data=df_republicans)

display(estimate_contrasts(m, contrast="Intervention", p_adjust="none"))

estimate_means(m, by = "Intervention") |> 
  ggplot(aes(x=Intervention, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MOCRI (dprime difference post-pre)", x="Intervention vs Control")

```

## ONLY SELF-IDENTIFIED REPUBLICANS [party affiliation] Intervention vs Control (independent/predictor) & MIST COVID (% Correct difference post-pre) (dependent/outcome)

```{r}
df_republicans <- df[df$Political_Affiliation == "Republican", ]

m <- lm(MIST_correct_Diff_covid ~ Intervention, 
        data=df_republicans)

display(estimate_contrasts(m, contrast="Intervention", p_adjust="none"))

estimate_means(m, by = "Intervention") |> 
  ggplot(aes(x=Intervention, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MIST-Covid (% Correct difference post-pre)", x="Intervention vs Control")

```

## ONLY SELF-IDENTIFIED REPUBLICANS [party affiliation] Intervention vs Control (independent/predictor) & MIST COVID (dprime difference post-pre) (dependent/outcome)

```{r}
df_republicans <- df[df$Political_Affiliation == "Republican", ]

m <- lm(MIST_dprime_Diff_covid ~ Intervention, 
        data=df_republicans)

display(estimate_contrasts(m, contrast="Intervention", p_adjust="none"))

estimate_means(m, by = "Intervention") |> 
  ggplot(aes(x=Intervention, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MIST-Covid (dprime difference post-pre)", x="Intervention vs Control")

```


## ONLY SELF-IDENTIFIED REPUBLICANS [party affiliation] Intervention vs Control (independent/predictor) & MIST GENERAL (% Correct difference post-pre) (dependent/outcome)

```{r}
df_republicans <- df[df$Political_Affiliation == "Republican", ]

m <- lm(MIST_correct_Diff_general ~ Intervention, 
        data=df_republicans)

display(estimate_contrasts(m, contrast="Intervention", p_adjust="none"))

estimate_means(m, by = "Intervention") |> 
  ggplot(aes(x=Intervention, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MIST-general (% Correct difference post-pre)", x="Intervention vs Control")
```


## ONLY SELF-IDENTIFIED REPUBLICANS [party affiliation] Intervention vs Control (independent/predictor) & MIST GENERAL (dprime difference post-pre) (dependent/outcome)

```{r}
df_republicans <- df[df$Political_Affiliation == "Republican", ]

m <- lm(MIST_dprime_Diff_general ~ Intervention, 
        data=df_republicans)

display(estimate_contrasts(m, contrast="Intervention", p_adjust="none"))

estimate_means(m, by = "Intervention") |> 
  ggplot(aes(x=Intervention, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MIST-general (dprime difference post-pre)", x="Intervention vs Control")
```


## ONLY SELF-IDENTIFIED REPUBLICANS [party affiliation] Intervention vs Control (independent/predictor) & MIST TOTAL (% Correct difference post-pre) (dependent/outcome)

```{r}
df_republicans <- df[df$Political_Affiliation == "Republican", ]

m <- lm(MIST_correct_Diff ~ Intervention, 
        data=df_republicans)

display(estimate_contrasts(m, contrast="Intervention", p_adjust="none"))

estimate_means(m, by = "Intervention") |> 
  ggplot(aes(x=Intervention, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MIST-TOTAL (% Correct difference post-pre)", x="Intervention vs Control")
```

## ONLY SELF-IDENTIFIED REPUBLICANS [party affiliation] Intervention vs Control (independent/predictor) & MIST TOTAL (dprime difference post-pre) (dependent/outcome)

```{r}
df_republicans <- df[df$Political_Affiliation == "Republican", ]

m <- lm(MIST_dprime_Diff ~ Intervention, 
        data=df_republicans)

display(estimate_contrasts(m, contrast="Intervention", p_adjust="none"))

estimate_means(m, by = "Intervention") |> 
  ggplot(aes(x=Intervention, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MIST-TOTAL (dprime difference post-pre)", x="Intervention vs Control")
```


## ONLY SELF-IDENTIFIED CONSERVATIVES [ideology affiliation] Intervention vs Control (independent/predictor) & MIST COVID (% Correct difference post-pre) (dependent/outcome)

```{r}
df_conservatives <- df[df$Political_LiberalConservative >= 5 & df$Political_LiberalConservative <= 7, ]

m <- lm(MIST_correct_Diff_covid ~ Intervention, 
        data=df_conservatives)

display(estimate_contrasts(m, contrast="Intervention", p_adjust="none"))

estimate_means(m, by = "Intervention") |> 
  ggplot(aes(x=Intervention, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MIST-Covid (% Correct difference post-pre)", x="Intervention vs Control")

```

## ONLY SELF-IDENTIFIED CONSERVATIVES [ideology affiliation] Intervention vs Control (independent/predictor) & MIST General (% Correct difference post-pre) (dependent/outcome)

```{r}
df_conservatives <- df[df$Political_LiberalConservative >= 5 & df$Political_LiberalConservative <= 7, ]

m <- lm(MIST_correct_Diff_general ~ Intervention, 
        data=df_conservatives)

display(estimate_contrasts(m, contrast="Intervention", p_adjust="none"))

estimate_means(m, by = "Intervention") |> 
  ggplot(aes(x=Intervention, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MIST-General (% Correct difference post-pre)", x="Intervention vs Control")

```

## ONLY SELF-IDENTIFIED CONSERVATIVES [ideology affiliation] Intervention vs Control (independent/predictor) & MIST TOTAL (% Correct difference post-pre) (dependent/outcome)

```{r}
df_conservatives <- df[df$Political_LiberalConservative >= 5 & df$Political_LiberalConservative <= 7, ]

m <- lm(MIST_correct_Diff ~ Intervention, 
        data=df_conservatives)

display(estimate_contrasts(m, contrast="Intervention", p_adjust="none"))

estimate_means(m, by = "Intervention") |> 
  ggplot(aes(x=Intervention, y=Mean)) +
  geom_line(aes(group=1)) + 
  geom_pointrange(aes(ymin=CI_low, ymax=CI_high)) +
  labs(y="MIST TOTAL (% Correct difference post-pre)", x="Intervention vs Control")

```


## CONFIDENCE [political ideology]

```{r}
df_long <- df |> 
  pivot_longer(cols = starts_with("MIST_Confidence_"), 
               names_to = "Category", values_to = "Confidence") |> 
  mutate(TestPhase = ifelse(grepl("Pretest", Category), "Pretest", "Posttest"))

# Filter pretest data
df_pre <- filter(df_long, TestPhase == "Pretest")

# Run model
m_pre <- lm(Confidence ~ poly(Political_LiberalConservative, 2) * Category, 
            data = filter(df_pre, !is.na(Political_LiberalConservative)))

# Print model summary (this will show the p-values)
summary(m_pre)

# Print the summary of the pretest model, including overall significance
summary(m_pre)

# Print the p-value for the F-statistic (overall model)
summary(m_pre)$fstatistic


# Predict values
preds_pre <- estimate_relation(m_pre, length=30)

# Plot
ggplot(preds_pre, aes(x=Political_LiberalConservative, y=Predicted, color=Category)) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high, fill=Category), alpha=0.1) +
  geom_line(size=1) +
  labs(y="Confidence", x="Political Identification", title="Pretest Confidence") +
  theme_minimal()


##___________________________________________________________

# Filter posttest data
df_post <- filter(df_long, TestPhase == "Posttest")

# Run model
m_post <- lm(Confidence ~ poly(Political_LiberalConservative, 2) * Category, 
             data = filter(df_post, !is.na(Political_LiberalConservative)))

# Print model summary (this will show the p-values)
summary(m_post)

# Predict values
preds_post <- estimate_relation(m_post, length=30)

# Plot
ggplot(preds_post, aes(x=Political_LiberalConservative, y=Predicted, color=Category)) +
  geom_ribbon(aes(ymin=CI_low, ymax=CI_high, fill=Category), alpha=0.1) +
  geom_line(size=1) +
  labs(y="Confidence", x="Political Identification", title="Posttest Confidence") +
  theme_minimal()


```

## CONFIDENCE [intervention vs control] pretest and posttest

```{r}
df_long <- df |> 
  pivot_longer(cols = starts_with("MIST_Confidence_"), 
               names_to = "Category", values_to = "Confidence") |> 
  mutate(TestPhase = ifelse(grepl("Pretest", Category), "Pretest", "Posttest"))

# Filter pretest data
df_pre <- filter(df_long, TestPhase == "Pretest")

# Run model for pretest, using 'Intervention' instead of political ideology
m_pre <- lm(Confidence ~ Intervention * Category, 
            data = filter(df_pre, !is.na(Intervention)))

# Print model summary for pretest
summary(m_pre)

# Predict values for pretest
preds_pre <- estimate_relation(m_pre, length=30)

# Plot for pretest
ggplot(preds_pre, aes(x=Intervention, y=Predicted, fill=Category)) +
  geom_boxplot() +
  labs(y="Confidence", x="Intervention Group", title="Pretest Confidence by Intervention Group") +
  theme_minimal()

##___________________________________________________________

# Filter posttest data
df_post <- filter(df_long, TestPhase == "Posttest")

# Run model for posttest, using 'Intervention' instead of political ideology
m_post <- lm(Confidence ~ Intervention * Category, 
             data = filter(df_post, !is.na(Intervention)))

# Print model summary for posttest
summary(m_post)

# Predict values for posttest
preds_post <- estimate_relation(m_post, length=30)

# Plot for posttest
ggplot(preds_post, aes(x=Intervention, y=Predicted, fill=Category)) +
  geom_boxplot() +
  labs(y="Confidence", x="Intervention Group", title="Posttest Confidence by Intervention Group") +
  theme_minimal()

```

## CONFIDENCE [intervention group only] confidence and time


```{r}
# Linear model for MIST Confidence difference based on Intervention, Pretest to Posttest
m_intervention <- lm(MIST_Confidence_Posttest - MIST_Confidence_Pretest ~ Intervention, 
                     data = df)

# Estimate contrasts (for significance testing between intervention and control)
display(estimate_contrasts(m_intervention, contrast="Intervention", p_adjust="none"))

# Estimate means by intervention group
estimate_means(m_intervention, by = "Intervention") |> 
  ggplot(aes(x = Intervention, y = Mean)) +
  geom_line(aes(group = 1)) + 
  geom_pointrange(aes(ymin = CI_low, ymax = CI_high)) +
  labs(y = "MIST Confidence Difference (Posttest - Pretest)", x = "Intervention vs Control")

```


```{r}
# # Subset the data for just the Intervention group
# df_intervention <- filter(df, Intervention == "Intervention")
# 
# # Pivot the data to long format so 'Time' can be a factor
# df_intervention_long <- df_intervention %>%
#   pivot_longer(cols = c(MIST_Confidence_Pretest, MIST_Confidence_Posttest),
#                names_to = "Time", values_to = "Confidence")
# 
# # Linear model for MIST Confidence difference within the Intervention group
# m_intervention <- lm(Confidence ~ Time, data = df_intervention_long)
# 
# # Print the model summary to see if the difference between pretest and posttest is significant
# summary(m_intervention)
# 
# # Plotting the difference between pretest and posttest confidence for the Intervention group
# ggplot(df_intervention_long, aes(x = Time, y = Confidence)) + 
#   geom_boxplot() +
#   labs(y = "MIST Confidence", x = "Test Phase (Pretest vs Posttest)") + 
#   ggtitle("MIST Confidence in Intervention Group (Pretest vs Posttest)")

```


```{r}
#browseURL("analysis_report.html")  # Use this in the analysis file

```

